...
#$$strtCprt
"""
* Repetition Timer (Full Body Track) VR
* 
...
class SceneGenerator(object):
    ...
    def __init__(self):
        ...
        self.idealWeightHits = 0;
        self.prevIdealWeightHits = -1;
        ...
        # create the ideal weight geometry
        self.idealWeightSizeY = 0.16;
        self.idealWeightSizeXZ = 0.46;
        self.idealWeightSizeGeom = Vec3.Vec3()
        self.idealWeightSizeGeom.set(
            self.idealWeightSizeXZ,
            self.idealWeightSizeY,
            self.idealWeightSizeXZ
        );
        ...
        self.idealWeightAColor = Vec3.Vec3()
        self.idealWeightAColor.set( 0xf1 / 255.0 , 0xed / 255.0 , 0xc2 / 255.0 )
        ...
        self.idealWeightsA = [None] * self.maxIdealWeights;
        for cnt in range( len( self.idealWeightsA ) ) :
            self.idealWeightsA[cnt] = Target.Target();
            tobj = self.idealWeightsA[cnt];
            if cnt == 0 : 
                tobj.strtPos.set(-LFT_X_OFFSET, 1.8, 0.0);
                tobj.endPos.set(-LFT_X_OFFSET, 1.8, 0.0);
            else:
                tobj.strtPos.set(+LFT_X_OFFSET, 1.8, 0.0);
                tobj.endPos.set(+LFT_X_OFFSET, 1.8, 0.0);
            tobj.mesh = SingleColorBox.SingleColorBox()
            if cnt == 0 :
                tobj.mesh.posn.set(-LFT_X_OFFSET, 1.8, 0.0)
            else:
                tobj.mesh.posn.set(+LFT_X_OFFSET, 1.8, 0.0)
            tobj.mesh.color = self.idealWeightAColor
            tobj.mesh.size = self.idealWeightSizeGeom
            tobj.rotationRate = 0.01;
            tobj.strtTime = self.utimeStrt;
            tobj.endTime = self.utimeEnd;
            tobj.timeEnabled = False;
        ...
        # Strategies moved to RepTimerActor
    ...
    def generate(self):
        ...
        for cnt in range( len( self.idealWeightsA ) ) :
            self.idealWeightsA[cnt].mesh.genPrev(ret)
        # for
    ...
    def checkFootHurdleCollision(self, controllerMesh : iRenderGen.IRenderGen , tobj : Target.Target ):
        ...
        if not (tobj.collided) : 
            ...
            sizeConstraintXZ = (self.controllerBaseSize) * 0.5 + (self.idealWeightSizeXZ) * 0.5;

            sizeConstraintY = (self.controllerBaseSize) * 0.5 + (self.idealWeightSizeY) * 0.5;
